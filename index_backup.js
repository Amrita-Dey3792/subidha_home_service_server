...
console.log("ImageBB API Key:", IMAGEBB_API_KEY);
...
async function uploadToImageBB(imageBuffer, filename) {
  try {
    ...
    console.log(`API Key: ${IMAGEBB_API_KEY.substring(0, 10)}...`);
    ...
    console.log(`ImageBB response for ${filename}:`, response.data);

    if (response.data.success) {
      console.log(
        `✅ Successfully uploaded ${filename}: ${response.data.data.url}`
      );
      return {
        success: true,
...
async function run() {
  ...
  try {
      ...
      try {
        ...
        if (existingUser) {
          ...
          if (existingUser.role && existingUser.role !== "customer") {
            ...
            console.log(
              `Preserving existing role '${existingUser.role}' for user ${user.uid}`
            );
          }
    ...
    app.delete("/users/:id", async (req, res) => {
      try {
        ...
        if (user.uid && admin.apps.length > 0) {
          ...
          console.log(
            "Firebase Admin SDK not initialized - skipping Firebase deletion"
          );
          firebaseError = "Firebase Admin SDK not available";
        } else {
          console.log("User has no UID - skipping Firebase deletion");
        }
    ...
    io.on("connection", (socket) => {
      ...
      socket.on("joinRoom", async ({ uid1, uid2, bookingId }) => {
        // Ensure that the room ID is unique for the conversation
        let roomId;

        // Generate consistent room ID with booking context
        const participants = [uid1, uid2].sort();
        const baseRoomId = `chat_${bookingId || "default"}_${participants[0]}_${
          participants[1]
        }`;
        console.log(
          `Generated room ID: ${baseRoomId} for participants: ${uid1}, ${uid2}`
        );
        ...
        if (result) {
          ...
          roomId = baseRoomId;
          console.log(`Creating new room ID: ${roomId}`);
        }
        // Check if the room already has two participants
        const roomMembers = roomParticipants[roomId] || [];
        if (roomMembers.length < 2) {
          console.log(`Socket ${socket.id} joining room: ${roomId}`);
          socket.join(roomId);

          // Add the participant to the list
          roomParticipants[roomId] = roomMembers.concat(socket.id);
          console.log(
            `Room ${roomId} now has participants:`,
      ...
      socket.on("typing", ({ roomId, senderId, receiverId }) => {
        io.to(roomId).emit("typing", { senderId, receiverId, roomId });
      });

      socket.on("notTyping", ({ roomId, senderId, receiverId }) => {
        io.to(roomId).emit("notTyping", { senderId, receiverId, roomId });
      });
...
app.post("/api/bookings/:bookingId/accept", async (req, res) => {
  try {
    ...
    if (
      ...
      console.log("Booking status check failed:", {
        expectedStatus: "broadcast_pending",
        actualStatus: booking.status,
        expectedBroadcastStatus: "active",
        actualBroadcastStatus: booking.broadcastStatus,
      });

      // Temporary: Allow non-broadcast bookings for testing
      console.log("Allowing non-broadcast booking for testing");
    }
    ...
    console.log("Provider eligibility check:", {
      ...
      providerServiceCategories: provider.serviceCategories,
      providerServices: provider.services,
      providerMyServices: provider.myServices,
      service: service
        ? {
            _id: service._id,
            serviceName: service.serviceName,
      ...
      bookingBroadcastedProviders: booking.broadcastedProviders,
    });
